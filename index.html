  ...
  const createChannel = () => {
    ...
    setServers(prev => prev.map(s => {
        if (s.id === activeServerId) {
            return {
                ...s,
                channels: [...s.channels, { id: newChanId, name: newChannelName.toLowerCase().replace(/\s+/g, '-'), type: 'text' }]
            };
        }
        return s;
    }));
            ...
            <>
                ...
                <div className="flex-1 overflow-y-auto py-3 custom-scrollbar">
                    <div className="px-4 mb-2 flex items-center justify-between group text-gray-400 hover:text-gray-300 cursor-pointer">
                        ...
                        {isOwner ? (
                             <Plus size={14} className="opacity-0 group-hover:opacity-100" onClick={() => setActiveModal('createChannel')} />
                        ) : <div />}
                    </div>
                    <div className="space-y-[2px]">
                        ...
                        >
                            {channel.type === 'voice' ? <Volume2 size={20} className="mr-1.5 text-gray-400" /> : <Hash size={20} className="mr-1.5 text-gray-400" />}
                            <span className={`truncate font-medium ${activeChannelId === channel.id || voiceState.channelId === channel.id ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'}`}>
                                {channel.name}
                            </span>
                        </div>
                        ))}
          ...
          <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
            <div className="bg-[#313338] text-white rounded-lg shadow-2xl w-full max-w-md p-6">
                 ...
                 <div className="flex justify-end space-x-2">
                     <button onClick={() => setActiveModal(null)} className="px-4 py-2 hover:underline">Cancel</button>
                     <button onClick={createChannel} className="px-6 py-2 bg-indigo-500 hover:bg-indigo-600 rounded font-medium">Create Channel</button>
                 </div>
            </div>
          </div>
      )}
